...
可直接运行的 Web 演示：LLM 驱动的民俗悬疑推理。玩家误入被红雾封住的月隐村，必须在“宿狼祭”的白天审判与夜晚闭门之间建立信任、收集线索，查清十二年前月蚀夜的旧案。服务端读取 `.env` 中的 LLM 基础地址、API Key、模型名，转发 `/api/chat` 请求，并要求模型按“先 JSON 再文本”的结构化输出。
...
- 真相卡在 `data/truthCard.js`，当前设定为原创“月隐村 / 宿狼祭 / 狼人疑云”背景，可继续修改场景皮肤、NPC 秘密和通关物品。
